...
export const EditTournamentModal: React.FC<EditTournamentModalProps> = ({
  ...
  const { updateTournament, deleteTournament } = useApp();
  ...
  // Sync state whenever selected tournament changes
  useEffect(() => {
    if (tournament) {
      setTitle(tournament.title || '');
      setDescription(tournament.description || '');
      setRules(tournament.rules || '');
      setEntryFee(tournament.entry_fee ?? 100);
      setPrizeChampion(tournament.prize_champion ?? 600);
      setPrizeRunnerup(tournament.prize_runnerup ?? 400);
      setPrize2ndRunnerup(tournament.prize_2nd_runnerup ?? 200);
      setRefundSemifinal(tournament.refund_semifinal ?? 100);
      setMaxPlayers(tournament.max_players ?? 32);
      setStatus(tournament.status || 'upcoming');
      setBkashNumber(tournament.bkash_number || '01712-893421');
      setNagadNumber(tournament.nagad_number || '01844-592019');
      setRocketNumber(tournament.rocket_number || '01923-118844-2');
      setTournamentCode(tournament.tournament_code || '');
      setBannerUrl(tournament.banner_url || PRESET_COVERS[0].url);

      if (tournament.start_time) {
        try {
          const dateObj = new Date(tournament.start_time);
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
            ...
            <div>
              <h3 style={{ fontSize: '1.15rem' }}>Edit Tournament</h3>
              <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                Update cover banner, prize pool, format, and receiving numbers
              </span>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
              ...
              <span style={{ fontSize: '0.725rem', color: 'var(--text-muted)' }}>
                Live across homepage, details, & player cards
              </span>
            </div>
            ...
            <div>
              <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', display: 'block', marginBottom: '0.4rem' }}>
                Or select from curated esports presets:
              </span>
              {/* Preset Covers Grid */}
          ...
          {/* Grid for Status, Entry Fee & Max Players */}
          ...
          <div style={{ background: 'rgba(10, 16, 30, 0.6)', padding: '1rem', borderRadius: '0px', border: '1px solid var(--border-subtle)' }}>
            <span style={{ fontSize: '0.8rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--accent-amber)', display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.75rem' }}>
              <Trophy size={14} /> Prize Pool Breakdown (BDT)
            </span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 200px), 1fr))', gap: '1rem' }}>
            ...
            <div className="form-group">
              <label className="form-label">Match Room Code</label>
              <input
                type="text"
                className="form-input mono"
          ...
          <div style={{ background: 'rgba(10, 16, 30, 0.6)', padding: '1rem', borderRadius: '0px', border: '1px solid var(--border-subtle)' }}>
            <span style={{ fontSize: '0.8rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--accent-cyan)', display: 'block', marginBottom: '0.75rem' }}>
              Receiving Mobile Banking Numbers
            </span>
          ...
          {showDeleteConfirm ? (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--accent-rose)', fontSize: '0.85rem' }}>
                ...
                <span>Permanently delete this tournament and its registrations?</span>
              </div>
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <button
                  type="button"
